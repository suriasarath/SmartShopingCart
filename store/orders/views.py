...
def order_completed1(request,order):
    order_number =  order #request.GET.get('order_number')
    #transID = request.GET.get('payment_id')
    print(order_number,"------")
    try:
...
def return_order(request,order):
    ...
    return render(request, 'shop/orders/order_completed/order_returned.html', context)
